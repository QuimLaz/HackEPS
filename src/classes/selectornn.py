...
class Selector:
    ...
    def __init__(self, learning_rate):
        ...
        self.nn = Sequential()
        self.nn.add(Dense(50, 'sigmoid', N_INPUTS))
        self.nn.add(Dense(50, 'relu'))
        self.nn.add(Dense(1, activation='sigmoid'))
        adam = keras.optimizers.Adam(lr=self.learning_rate, beta_1=0.9, beta_2=0.999, epsilon=None, decay=0.0, amsgrad=False)
        self.nn.compile(loss='binary_crossentropy', optimizer=adam, metrics=['accuracy'])

    def compare(self, task1, task2):
        output = self.nn.predict(np.append(get_parameters(task1), get_parameters(task2)))
    ...
    def update_params(self, true_solution_id):
        ...
        x_train = np.zeros(shape=(n, N_INPUTS))
        y_train = np.zeros(shape=(n, 1))
        for i, task in enumerate(self.last_scenario.tasks):
            if task.id != true_solution.id:
                x_train[i, :] = np.append(get_parameters(true_solution), get_parameters(task))
                y_train[i] = 1
                x_train[i + n, :] = np.append(get_parameters(task), get_parameters(true_solution))
                y_train[i + n] = 0
